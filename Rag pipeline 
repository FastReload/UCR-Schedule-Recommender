...
- Include prerequisites in the prompt so the LLM can reason about eligibility.
...
- Agent Builder does **not inherently “know” prerequisite logic.**
- Instead:
  - Prerequisites are stored as metadata in each course document.
  - When Agent Builder retrieves a course, it includes the prerequisite info.
- It’s the LLM’s job to:
  - Read the student’s completed courses.
  - Compare them to prerequisites in the prompt.
  - Decide whether to recommend each course.
- This prevents recommending courses the student isn’t eligible to take.
- **Example:**
  ```
...
- Agent Builder retrieves them.
- The LLM reasons about whether the student meets them.
